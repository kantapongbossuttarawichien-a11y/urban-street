...
export function RushHourChart({ data, isLoading }: RushHourChartProps) {
  ...
  return (
    <div className="bg-white p-5 sm:p-10 rounded-3xl sm:rounded-[3rem] border border-stone-100 shadow-sm relative overflow-hidden group h-full">
      <div className="flex justify-between items-end mb-6 sm:mb-10">
        <div>
          <h4 className="text-sm font-black uppercase tracking-[0.1em] text-black">ช่วงเวลาขายดี</h4>
          <p className="text-[9px] sm:text-[10px] font-bold text-stone-300 uppercase tracking-tighter mt-1">ความถี่ของออเดอร์ในแต่ละชั่วโมง</p>
        </div>
        <div className="flex gap-2 items-center">
           <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-black" />
           <span className="text-[9px] sm:text-[10px] font-black text-stone-400 uppercase tracking-wide sm:tracking-widest">ปริมาณออเดอร์</span>
        </div>
      </div>

      <div className="flex items-end justify-between h-44 sm:h-56 gap-2 sm:gap-3">
        {isLoading ? (
          ...
          data.map((d, i) => (
            <div key={d.hour} className="flex-1 flex flex-col items-center gap-2 sm:gap-4 h-full justify-end group/bar">
              ...
              <span className="text-[9px] sm:text-[10px] font-black text-stone-300 uppercase tracking-tight">
                {d.hour}
              </span>
            </div>
          ))
